require 'xmlsimple'
module Jiraissues
    def Jiraissues.fetch_issue(config_hash, jira_id)

      command = "curl -s --cert #{config_hash['PERSONAL_PEM']} --cacert #{config_hash['CA_PEM']}"

      # append the querystring from the config onto the jira urls
      issue_list_xml_url = "https://jira.dev.bbc.co.uk/si/jira.issueviews:issue-xml/#{jira_id}/#{jira_id}.xml"
      ...
      xml_obj['channel'][0]['item'].each do |item| 
        issue = {
          ...
          :firstcomment => fetch_first_comment(item['comments'])
        }
        keys.push issue
      end

      {:keys => keys, :url => issue_list_xml_url}
    ...
    def Jiraissues.fetch_field(fieldArray, fieldID)
      ...
      value
		...
		def Jiraissues.fetch_first_comment(commentsArray)
      if commentsArray == nil
        return ""
      end                                                
      last_comment = commentsArray[0]['comment'][0]
      { "created" => last_comment['created'], "comment"=> last_comment['content'] }
    end    
